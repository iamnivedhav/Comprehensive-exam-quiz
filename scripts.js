...
const loadQuestions = async (config = null) => {
    const loaderElement = document.getElementById('quiz-container') || document.getElementById('quiz-interface') || document.getElementById('study-list-container');
    const loadingMessage = config ? "Loading data..." : "Loading question data...";
    ...
    try {
        ...
        if (!Array.isArray(allQuestions) || allQuestions.length === 0) {
             loaderElement.innerHTML = `<div class="text-center p-8 header-box shadow-neon-red"><p class="text-red-400 font-bold text-xl">Data Error: File is Empty or Invalid</p></div>`;
             return;
        }

        if (config && config.mode === 'study') {
            renderStudyList(config.subject);
        } else if (config) {
            prepareQuiz(config.mode, config.subject);
        } else {
            renderStartScreen();
        }
    ...
    quizContainer.innerHTML = `
        <div class="space-y-8">
            ...
            <div class="p-6 header-box neon-border shadow-neon-blue">
                <h2 class="text-2xl font-bold text-neon-blue mb-4 border-b border-gray-600 pb-2">Subject-Specific Modes</h2>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <button id="study-mode-btn" onclick="startStudy()"
                        class="neon-button-blue disabled:opacity-50" disabled>
                        Study Mode (Scroll/Review)
                    </button>
                </div>
            </div>
        </div>
...
const startStudy = () => {
    ...
    if (subject) {
        window.location.href = `study.html?subject=${encodeURIComponent(subject)}`;
    }
};
...
// --- Study Mode Logic (study.html) ---

const renderStudyList = (subject) => {
    const studyContainer = document.getElementById('study-list-container');
    const filteredQuestions = allQuestions.filter(q => q.topic === subject);
    
    if (filteredQuestions.length === 0) {
        studyContainer.innerHTML = `<p class="text-red-400 text-center text-xl p-8 header-box">No questions found for the subject: ${subject}</p>`;
        return;
    }

    document.getElementById('question-count-text').textContent = `${filteredQuestions.length} Questions for Review`;
    
    const studyContent = filteredQuestions.map(q => {
        const correctAnswerIndex = q.answerIndex;
        const optionsHtml = q.options.map((optionText, index) => {
            const isCorrect = index === correctAnswerIndex;
            const style = isCorrect ? 'option-correct' : '';
            return `
                <div class="w-full text-left py-2 px-4 rounded-lg text-base ${style} bg-gray-700/50">
                    ${optionText}
                </div>
            `;
        }).join('');
        ...
        return `
            <div class="header-box neon-border p-6 md:p-8 shadow-neon-blue/50">
                <div class="flex justify-between items-start mb-4 border-b border-gray-700 pb-3">
                    <p class="text-xl text-neon-blue font-bold">Topic: ${q.topic}</p>
                    <p class="text-md text-gray-400">ID: ${q.id}</p>
                </div>
                <div class="question-box p-4 rounded-xl mb-6">
                    <p class="text-lg font-semibold leading-relaxed">${q.question.replace(/\n/g, '<br>')}</p>
                </div>
                <div class="space-y-2">
                    ${optionsHtml}
                </div>
    ...
    studyContainer.innerHTML = studyContent;
};
